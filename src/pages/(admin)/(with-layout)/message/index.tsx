import type { ColumnFiltersState, ExpandedState, PaginationState } from "@tanstack/react-table"
import {
  flexRender,
  getCoreRowModel,
  getExpandedRowModel,
...
import { CircleCheck, CircleX, Clock, Hourglass, MessageCircleDashed, RefreshCwIcon, Trash, TriangleAlert, UserRound, X } from "lucide-react"
...
import type { MessageStatus } from "@/schema/message"
...
import { order_columns } from "./components/order-table-columns"
...
export function Component() {
  ...
  const today = useNow(1, "date").toLocaleDateString("zh-CN")
            ...
            <TableHeader className="bg-muted/50 font-medium">
              <TableRow>
                <TableCell colSpan={table.getVisibleFlatColumns().length}>
                  页面小计:
                  {CURRENCY_FORMAT.format(messages.summary?.page_total ?? 0)}
                </TableCell>
              </TableRow>
            </TableHeader>
                ...
                <TableRow key={headerGroup.id}>
                  {headerGroup.headers.map((header) => (
                    <TableHead
                      ...
                      className={clsx({
                        "w-14": ["select", "status", "actions", "user_id", "amount"].includes(header.column.id),
                        "max-w-14": ["select", "status", "actions"].includes(header.column.id),
                        "w-48 max-w-48": header.column.id === "id",
                        "w-24 max-w-40": ["ts"].includes(header.column.id),
                      })}
                    >
                      ...
                      >
                        ...
                        <TableCell colSpan={row.getVisibleCells().length - 1}>
                          <div className="mb-2 mr-3">
                            <OrderTable
                              columns={order_columns.filter((c) => c.id !== "select")}
